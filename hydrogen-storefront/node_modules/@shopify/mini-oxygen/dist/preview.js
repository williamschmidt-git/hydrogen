...
async function preview(opts) {
    ...
    log = (message) => console.log(message), port = 3000, workerFile, assetsDir, publicPath, watch = false, buildWatchPaths, buildCommand, autoReload = false, modules = true, sourceMap = true, proxyServer, envPath, env = {}, oxygenHeaders, onRequest, onResponseError, onResponse = (req, res) => {
        log(`${req.method}  ${res.status}  ${req.url.replace(new URL(req.url).origin, '')}`);
    }, } = opts;
    ...
    const actualPort = await (0, get_port_1.default)({ port });
    if (actualPort !== port) {
